...
function buildNode(id) {
    ...
    if (model.isNodeStart(id)) {
        ...
        builder.createPath(startContainer, "", dValue, SIZE.pathHidden, "", COLOR.transparent, true);
        builder.createPath(startContainer, "", dValue, SIZE.path, CONSTANTS.arrow, COLOR.white, true);
    }
    ...
    if (model.isNodeEnd(id)) {
        builder.createCircle(container, node.coords, SIZE.nodeRadius - 1);
    }
...
export function reset() {
    ...
    builder.createMarker(defs, CONSTANTS.arrow, 16, 10, DISTANCE.markerOffset, 5, COLOR.white, polygon);
    builder.createMarker(defs, CONSTANTS.arrowSelected, 16, 10, DISTANCE.markerOffset, 5, COLOR.marked, polygon);
    builder.createMarker(defs, CONSTANTS.selfarrow, 16, 16, DISTANCE.markerSelfOffset, DISTANCE.markerSelfOffsetHeight, COLOR.white, selfPolygon);
    builder.createMarker(defs, CONSTANTS.selfarrowSelected, 16, 16, DISTANCE.markerSelfOffset, DISTANCE.markerSelfOffsetHeight, COLOR.marked, selfPolygon);
    ...
    builder.createPath(svg, CONSTANTS.defaultPath, "", SIZE.path, CONSTANTS.defaultMarker, COLOR.marked);
...
function buildEdges(id) {
    ...
    for (let otherNode in node.to) {
        ...
        if (id === nodeId) {
            ...
            const outerPath = builder.createPath(pathContainer, "", dValue, SIZE.pathHidden, "", COLOR.transparent, true);
            const innerPath = builder.createPath(pathContainer, "", dValue, SIZE.path, markerEnd, COLOR.white, true);
            ...
            builder.createPath(pathContainer, "", dValue, SIZE.pathHidden, "", COLOR.transparent, true);
            builder.createPath(pathContainer, "", dValue, SIZE.path, markerEnd, COLOR.white, true);
